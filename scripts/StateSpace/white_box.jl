using ASM1Simulator, DifferentialEquations, LinearAlgebra, Statistics, Optimization, OptimizationOptimisers, OptimizationOptimJL, Optim, Plots
...
function model(opt_params)
    ...
    for i in 1:5
        if i in index_obs_simplified_var
            ...
            X_init_sim[i] = opt_params[18 + i - (id_obs_var - 1)]
        end
    end
    ...
    try
        sol_sim = solve(remake(simplified_ode_prob, p=p_sim, u0=X_init_sim), choosen_integrator, saveat = tsave, callback=ASM1Simulator.Models.external_control(t_control, vcat(U_train...); index_u = 6), verbose=false, maxiters=Int(5*1e4))
...
res = optimize(u -> L2_loss(model(u), y_train[.! isnan.(y_train)]), p_init, optimizer, options)


#Optim.x_trace(res)
#plot(Optim.f_trace(res))

plot(1:dt_obs:Int(T_training*1440), res', label="Solution") # model(res.minimizer)
