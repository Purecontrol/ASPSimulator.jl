...
# @save "experiment_10.jld" res x_train y_train

# Quality reconstruction non-observed variable
res = load("experiment_4.jld")
opt_params = res["res"].minimizer
sol_opt_params = model(opt_params)
sol_theorical_params = model(theorical_opt_params)
id = 4
plot(1:dt_obs:Int(T_training*1440), sol_opt_params[id, :], label="Solution") # model(res.minimizer)
plot!(1:dt_obs:Int(T_training*1440), sol_theorical_params[id, :], label="Theoric")
plot!(sum(x_train[link_index_real_system[id], :], dims=1)', label="True state")
plot!(xformatter = x -> Dates.format(DateTime(2023, 1, 1) + Dates.Minute(x), "Jd - Hh"))

# Define ODEsim with the simplified model
current_t = T_steady_state + T_training
fixed_dt = sim.fixed_dt
ode_fct! = ASM1Simulator.Models.simplified_asm1!
state = sol_opt_params[:, end]
params_vec = deepcopy(simplified_real_p_vec)
params_vec[1] = opt_params[1:8]
params_vec[2] = opt_params[9:13]
params_vec[3] = ASM1Simulator.Models.get_stoichiometric_matrix_simplified_asm1(opt_params[14:16])
params_vec[4] = opt_params[17]
params_vec[7] = opt_params[18]
params_vec[8] = opt_params[19]
index_u  = [6]
sim2 = ODECore(current_t, fixed_dt, ode_fct!, state, params_vec, index_u)

prediction_simplified_model  = multi_step!(sim2, [[U_test[i, 1]] for i in 1:Int(T_testing*24*60)], Hour((T_testing)*24))
sol_simplified = prediction_simplified_model.u

params_vec2 = deepcopy(simplified_real_p_vec)
params_vec2[1] = theorical_opt_params[1:8]
params_vec2[2] = theorical_opt_params[9:13]
params_vec2[3] = ASM1Simulator.Models.get_stoichiometric_matrix_simplified_asm1(theorical_opt_params[14:16])
params_vec2[4] = theorical_opt_params[17]
params_vec2[7] = theorical_opt_params[18]
params_vec2[8] = theorical_opt_params[19]
sim3 = ODECore(current_t, fixed_dt, ode_fct!, state, params_vec2, index_u)

prediction_simplified_model2  = multi_step!(sim3, [[U_test[i, 1]] for i in 1:Int(T_testing*24*60)], Hour((T_testing)*24))
sol_simplified2 = prediction_simplified_model2.u

id = 3
plot(getindex.(sol_simplified, id), label="Solution")
plot!(getindex.(sol_simplified2, id), label="Theoric solution")
plot!(sum(x_test[link_index_real_system[id], :], dims=1)', label="True state")
# scatter!(y_test[2,:])
