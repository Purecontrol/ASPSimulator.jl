...
data = load("asm1-simulator/data/processed/asm1_estimation/PSO_3.jld")
...
Threads.@threads for ind in index
    ...
    x̂, part = EnKF(da2, ŷt, 30; progress = true, control = U)

    # Compute the RMSE
    rmse = sqrt.(mean((vcat(x̂.u'...) .- vcat(xt.u'...)) .^ 2, dims=1))
    println("RMSE: ", rmse)

    name_file = "asm1-simulator/data/processed/assimilation/EnKF-FP/SNR_model_$(SNR_model[i])_SNR_obs_$(SNR_obs[j]).jld"
