...
for i in eachindex(SNR_model)
    for j in eachindex(SNR_obs)
        ...
        x̂ = EnKF(da, ŷt, 30; progress = true, control = U)

        # Compute the RMSE
        rmse = sqrt.(mean((vcat(x̂.u'...) .- vcat(xt.u'...)) .^ 2, dims=1))
        println("RMSE: ", rmse)

        name_file = "asm1-simulator/data/processed/assimilation/EnKF/SNR_model_$(SNR_model[i])_SNR_obs_$(SNR_obs[j]).jld"
        @save name_file x̂ xt x̂t ŷt
    end
end
