...
function forecast(system::GaussianLinearStateSpaceSystem, current_state::GaussianStateStochasticProcess, exogenous_variables, control_variables, parameters; n_steps_ahead=1)
    ...
    current_H = system.H_t(exogenous_variables, parameters, current_state.t)
    ...
    predicted_obs[1].σ_t = transpose(current_H)*current_state.σ_t*current_H + system.Q_t(exogenous_variables, parameters, current_state.t)
    ...
    @inbounds for step in 2:(n_steps_ahead+1)
        ...
        A = system.A_t(exogenous_variables, parameters, t_step)
        B = system.B_t(exogenous_variables, parameters, t_step)
        H = system.H_t(exogenous_variables, parameters, t_step)
        
        # Update predicted state and covariance
        predicted_state[step].t = t_step
        predicted_state[step].μ_t = A*predicted_state[step-1].μ_t + B*control_variables[step-1, :] + system.c_t(exogenous_variables, parameters, t_step)
        predicted_state[step].σ_t = transpose(A)*predicted_state[step-1].σ_t*A + system.R_t(exogenous_variables, parameters, t_step)
        
        # Update observed state and covariance
        predicted_obs[step].t = t_step
        predicted_obs[step].μ_t = H*predicted_state[step].μ_t + system.d_t(exogenous_variables, parameters, t_step)
        predicted_obs[step].σ_t = transpose(H)*predicted_state[step].σ_t*H + system.Q_t(exogenous_variables, parameters, t_step)
