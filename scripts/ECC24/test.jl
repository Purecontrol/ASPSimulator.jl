...
# julia --project=JuliaEnvironment asm1-simulator/scripts/ECC24/test.jl nb_exp X_in
nb_exp = parse(Int, ARGS[1])
X_in_args = parse(Float64, ARGS[2])
...
@timed optim_params_pfbs_em, results_pfbs = SEM(model, y_train, E_train, U_train; lb=lb, ub=ub, n_filtering = 300, n_smoothing = 300, maxiters_em=35, optim_method=Opt(:LD_LBFGS, 5), maxiters=100);
...
# Save results
dir = "asm1-simulator/data/results_Xin_fixe/"*ARGS[2]*"/"
isdir(dir) || mkdir(dir)
filename = dir*ARGS[1]*".jld"
@save filename x_pred_em mean_em q_low_em q_high_em x_pred_pem x_true x_true_down index_true index_pred optim_params_pfbs_em optim_params_pem.u