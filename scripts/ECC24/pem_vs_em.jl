...
filter_output, _, _ = StateSpaceIdentification.filter(model, y_total, E_total, u_total, filter=ParticleFilter(model, n_particles = n_filtering));
...
# Plot predictions
plot(x_pred_em, label=["EM-PFBS"])
plot!(index_pred, x_pred_pem, label="PEM")
plot!(index_true, x_true, label="True state")

# Compare errors prediction
max_nh4 = max(maximum(mean_em), maximum(x_true_down), maximum(x_pred_pem));
plot(LinRange(0, max_nh4, 100), LinRange(0, max_nh4, 100), lw=3, xlabel=L"\hat{S}_{NH}", ylabel=L"S_{NH}", legend=:topleft)
scatter!(x_true_down, mean_em, markersize=2.5, label="EM")
scatter!(x_true_down, x_pred_pem, markersize=2.5, label="PEM")

#######################################################
############### Compute statistics models #############
#######################################################
...
# Compute statistics IC EM
IC_em = mean(q_low_em .< x_true_down .< q_high_em)
NW_em = mean(q_high_em - q_low_em)

# Compare RMSE models
interval=24
plot(collect(2:24), [rmse(h, x_true_down, mean_em, interval=interval) for h in 2:24], label="EM")
plot!(collect(2:24), [rmse(h, x_true_down, x_pred_pem, interval=interval) for h in 2:24], label="PEM")
