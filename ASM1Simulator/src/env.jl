...
DEFAULT_INFLUENT_FILE_PATH = "/home/victor/Documents/code/asm1-simulator/data/external/influent_files/dryinfluent.ascii"
...
mutable struct ODECore
    ...
    function ODECore()
        ...
        simulator = new(0.0, 1/1440, Models.asm1!, [], NamedTuple(), [14])
...
function reset!(env::ODECore; init_state=Nothing, params_vec=Nothing, influent_file_path=DEFAULT_INFLUENT_FILE_PATH)
    ...
    default_p, default_state_init = Models.get_default_parameters_asm1(influent_file_path=influent_file_path)
...
function multi_step!(env::ODECore, action::Union{Vector{Vector{Float64}}, CallbackSet}, n_steps::Union{Int64, Period})
    ...
    if isa(action, Vector{Vector{Float64}}) || isa(action, Vector{Vector{Int64}})
        ...
        # Update control index of init state with choosen action
        action = Models.external_control([env.current_t + (i-1)*env.fixed_dt for i in 1:n_steps], vcat(action...); index_u = env.index_u[1])
