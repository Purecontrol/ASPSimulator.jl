...
"""
function get_default_parameters_simplified_asm1(;
        T = 15, influent_file_path = nothing, fixed_concentration = nothing,
    ...
    if fixed_concentration ≠ nothing
        X_BH = fixed_concentration[1]
        X_BA = fixed_concentration[2]
        X_ND = fixed_concentration[3]
        X_S = fixed_concentration[4]
        S_S = fixed_concentration[5]
    else
        X_BH = 2238
        X_BA = 167
        X_ND = 3.10
    ...
    K_DCO = 10.0 * mean((X_S .+ S_S) ./ S_S)
    K_OH = 0.2
    K_NO = 0.5
    η_g = 0.8
    η_h = 0.8
    K_ND = 0.1 * mean(((X_S .+ S_S) ./ X_S) .* X_BH)
    K_NH = 1.0
    K_OA = 0.4
    kinetic_parameters = (K_DCO = K_DCO, K_OH = K_OH, K_NO = K_NO, η_g = η_g,
        η_h = η_h, K_ND = K_ND, K_NH = K_NH, K_OA = K_OA)
    ...
    if influent_file_path ≠ nothing && variable_inlet_concentration
        X_in = get_inlet_concentrations_from_src_files(
            influent_file_path, output_index = [[2, 4], 8, 9, 10, 11])
    else
    ...
    # Merge parameters
    p = merge(kinetic_parameters, additional_parameters,
        stoichiometric_parameters, other_params, exogenous_params)
...
function simplified_asm1!(dX, X, p::NamedTuple, t)
    ...
    dX[1:5] = (evaluate(p[20], t) / p[17]) * (evaluate.(p[21], t) - X[1:5]) +
              R * process_rates
    dX[6] = 0.0
...
function simplified_asm1!(dX, X, p::Array, t)
    ...
    # Compute differential equations
    dX[1:5] = (evaluate(p[20], t) / p[17]) * (evaluate.(p[21:25], t) - X[1:5]) +
              R * process_rates
    dX[6] = 0.0
    dX[2] += X[6] * p[19] * (p[18] - X[2])
end
